...
| `oracle_postgre/`       | A Python directory that contains code for your Dagster repository                 |
| `oracle_postgre_tests/` | A Python directory that contains tests for `oracle_postgre`                      |
...
pyenv virtualenv $PYTHON_VERSION oracle_postgre
pyenv activate oracle_postgre
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name oracle_postgre python=PYTHON_VERSION
conda activate oracle_postgre
```
...
Tests can be found in `oracle_postgre_tests` and are run with the following command:

```bash
pytest oracle_postgre_tests
```

As you create Dagster ops and graphs, add tests in `oracle_postgre_tests/` to check that your
